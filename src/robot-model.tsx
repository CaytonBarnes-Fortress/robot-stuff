...
export const RobotModel: React.FC<{ url: string; pitch: number; roll: number; heading: number, scale?: number, position?: THREE.Vector3 }> = ({
  url,
  pitch,
  roll,
  heading,
  ...
  useEffect(() => {
    const loadModel = async () => {
      ...
      switch (ext) {
        ...
        default:
          loadedObject = new THREE.Group().add(new THREE.Mesh(
