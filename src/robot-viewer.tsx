...
export const RobotViewer: React.FC<RobotViewerProps> = ({ modelUrl, pitch, roll, heading, style, className, showGrid, controls }) => {
  return (
    <div style={{ width: '100%', height: '100%', ...style }} className={className}>
      <Canvas gl={{ alpha: true }} camera={{ position: [1, 2, 3], fov: 50 }}>
        ...
        <RobotModel url={modelUrl} pitch={pitch} roll={roll} heading={heading} />
      </Canvas>
    </div>
  );
};
